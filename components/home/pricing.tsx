          ...
          {tiers.map((tier, index) => (
            ...
            >
              ...
              <div className="flex items-baseline gap-1 mb-4">
                ...
                {tier.price !== 'Custom' && (
                  <span className="text-xs font-mono uppercase opacity-70">/ month</span>
                )}
              </div>
              ...
              <ul className="flex-1 space-y-4 mb-8">
                {tier.features.map((feature, i) => (
                  ...
                  >
                    <Check
                      className={`w-4 h-4 ${tier.highlight ? 'text-yellow-400' : 'text-blue-700'}`}
                    />
                    {feature}
                  </li>
                ))}
              ...
              <button
                className={`w-full py-3 font-mono text-sm font-bold uppercase border border-border transition-all shadow-[4px_4px_0px_0px_var(--border)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none ${tier.highlight ? 'bg-white text-blue-700' : 'bg-blue-700 text-white'}`}
              >
                {tier.cta}
              </button>
            </div>
