...
export default function Features() {
  return (
    <section id="features" className="py-24 px-6 border-t border-black bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-black border border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
          {features.map((feature, index) => (
            <div key={index} className="bg-white p-8 group hover:bg-slate-50 transition-colors">
              <div className={`w-12 h-12 ${feature.color} border border-black flex items-center justify-center mb-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] group-hover:translate-x-[2px] group-hover:translate-y-[2px] group-hover:shadow-none transition-all`}>
                <feature.icon className="w-6 h-6 text-black" />
              </div>
              <h3 className="font-mono text-xl font-bold uppercase mb-2">{feature.title}</h3>
              <p className="font-mono text-sm text-slate-600 uppercase leading-tight">
                {feature.description}
              </p>
            </div>
          ))}
